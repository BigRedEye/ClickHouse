...
struct MatchImpl
{
        ...
        PaddedPODArray<UInt8> & res)
    {
        ...
        for (size_t i = 0; i < haystack_size; ++i)
        {
            ...
            {
                Searcher searcher(required_substr.data(), required_substr.size(), cur_haystack_offset - prev_haystack_offset);
            ...
            else
            {
                ...
                if (required_substr.empty())
                {
                    ...
                    else
                    {
                        res[i] = negate
                            ^ regexp.getRE2()->Match(
                                          {reinterpret_cast<const char *>(cur_haystack_data), cur_haystack_offset},
                                          0,
                ...
                else
                {
                    ...
                    if (match == cur_haystack_data + cur_haystack_offset)
                    {
                        // no match
                        res[i] = negate;
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            const size_t end_pos = haystack_offsets[i] - prev_haystack_offset - 1;
                            res[i] = negate
                                ^ regexp.getRE2()->Match(
                                              {reinterpret_cast<const char *>(cur_haystack_data), cur_haystack_offset},
                                              start_pos,
